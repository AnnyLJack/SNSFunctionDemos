...
@interface SCCaptureCameraController ()<UICollectionViewDelegate,UICollectionViewDataSource> {
    int alphaTimes;
...
@property (nonatomic, strong) PreviewPhotoCollectionView *photoCollectionView;
@end
...
- (void)addTopViewWithText:(NSString*)text {
    if (!_topContainerView) {
        ...
        videoBtn.frame = CGRectMake(topFrame.size.width - 100, 0 , 100, topFrame.size.height);
        [videoBtn setTitle:@"小视频" forState:UIControlStateNormal];
        videoBtn.backgroundColor = [UIColor clearColor];
        [videoBtn addTarget:self action:@selector(pushToMakeVideo:) forControlEvents:UIControlEventTouchUpInside];
        [_topContainerView addSubview:videoBtn];
...
- (void)pushToMakeVideo:(UIButton *)sender {
    SCNavigationController *nav = (SCNavigationController*)self.navigationController;
    if ([nav.scNaigationDelegate respondsToSelector:@selector(changeToTakeVideo:)]) {
        [nav.scNaigationDelegate changeToTakeVideo:self];
    }
}
...
- (void)addCameraMenuView {
    ...
    [self.bottomContainerView addSubview:menuView];
    self.cameraMenuView = menuView;
    [self addMenuViewButtons];
    //图片
    self.photoCollectionView = [[PreviewPhotoCollectionView alloc] initWithFrame:CGRectMake(0, CAMERA_MENU_VIEW_HEIGH, self.view.frame.size.width, self.bottomContainerView.frame.size.height - CAMERA_MENU_VIEW_HEIGH)];
    
    [self.bottomContainerView addSubview:self.photoCollectionView];
    
}

//拍照菜单栏上的按钮
- (void)addMenuViewButtons {
    NSMutableArray *normalArr = [[NSMutableArray alloc] initWithObjects:@"close_cha.png", @"shot.png", @"switch_camera.png", @"flashing_off.png", nil];
    NSMutableArray *highlightArr = [[NSMutableArray alloc] initWithObjects:@"close_cha_h.png", @"", @"", @"", nil];
    NSMutableArray *selectedArr = [[NSMutableArray alloc] initWithObjects:@"", @"shot_h.png", @"switch_camera_h.png", @"", nil];
    
    NSMutableArray *actionArr = [[NSMutableArray alloc] initWithObjects:@"dismissBtnPressed:", @"takePictureBtnPressed:", @"switchCameraBtnPressed:", @"flashBtnPressed:", nil];
