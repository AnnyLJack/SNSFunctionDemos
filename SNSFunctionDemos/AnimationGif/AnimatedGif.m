...
{   
    ...
    agqo.uiv = [[UIImageView alloc] init];
...
- (void) asynchronousLoading
{
    ...
    {
        AnimatedGifQueueObject *gifQueue = (AnimatedGifQueueObject *) [imageQueue objectAtIndex: 0];
    	NSData *data = [NSData dataWithContentsOfURL:gifQueue.url];
        imageView = gifQueue.uiv;
    	[self decodeGIF: data];
...
- (bool) GIFGetBytes: (long)length
{
    ...
    {
		GIF_buffer = [NSMutableData dataWithData:[GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)]];
        dataPointer += length;
		return YES;
	}
    else
