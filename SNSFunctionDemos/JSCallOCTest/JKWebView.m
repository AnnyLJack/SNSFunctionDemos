    ...
    if( self ){
        ...
        context[@"jakilllog"] = ^() {
            ...
            for (JSValue *jsVal in args) {
                NSLog(@"%@", jsVal);
                message = [jsVal toString];
            }
            JSValue *this = [JSContext currentThis];
            NSLog(@"this: %@",this);
            NSLog(@"-------End Log-------");
            
            UIAlertView *cusAlert = [[UIAlertView alloc] initWithTitle:@"" message:message delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            [cusAlert show];
